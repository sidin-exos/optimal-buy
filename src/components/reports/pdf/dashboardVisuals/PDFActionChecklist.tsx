...
const progressPct = Math.round((stats.done / tasks.length) * 100);
  ...
  <View style={styles.dashboardCard}>
    ...
    <View style={{ marginTop: 6, marginBottom: 10 }}>
      ...
      <View style={{ height: 6, backgroundColor: colors.surfaceLight, borderRadius: 3, overflow: "hidden", flexDirection: "row" }}>
        <View style={{ width: `${(stats.done / tasks.length) * 100}%`, height: 6, backgroundColor: colors.success }} />
        <View style={{ width: `${(stats.inProgress / tasks.length) * 100}%`, height: 6, backgroundColor: colors.warning }} />
      </View>
    </View>
    ...
    <View style={{ marginTop: 4 }}>
      {tasks.map((t, i) => (
        <View key={i} style={{ flexDirection: "row", alignItems: "flex-start", marginBottom: 6, paddingBottom: 6, borderBottomWidth: i < tasks.length - 1 ? 1 : 0, borderBottomColor: colors.border }}>
          ...
          {/* Task details */}
          <View style={{ flex: 1 }}>
            <Text style={{ fontSize: 8, color: colors.text, fontWeight: t.status === "Done" ? 400 : 500 }}>
              {t.task}
            </Text>
